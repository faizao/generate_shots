...
f = int(sys.argv[1])  #frequency


path = '/home/jonas/Desktop/dataset/vmodels_border/'
destino = '/home/jonas/Desktop/dataset/shots/georec{}/'.format(str(f))
...
def genshot(intervalo1,intervalo2):
    
    for nm in range(intervalo1,intervalo2+1):
        print (nm)
...
def main():
    #fiz isso para 40 processadores
    positions = []  #tenho cada posicao
    for i in range(1,2001,50):
        positions.append(i)
    
    
    for i in range(1,41): #dividindo em 40 processadores
        
        if i == 40:
            inicio = positions[i-1]
            fim = 2001
        else:
            inicio = positions[i-1]
            fim = positions[i]-1
        ...
        thread = Thread(target=genshot,args=[inicio,fim])
        thread.start()
